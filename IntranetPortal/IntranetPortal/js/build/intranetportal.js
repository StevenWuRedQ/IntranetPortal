...
angular.module('PortalApp').controller('MainCtrl', ['$rootScope', '$uibModal', '$timeout', function ($rootScope, $uibModal, $timeout) {
...
(function () {
    ...
    function portalRouteProvider($routeProvider) {
        ...
        this.routesFor = function (resourceName, urlPrefix, routePrefix) {
            ...
            var templateUrl = function templateUrl(operation) {
            ...
            var controllerName = function controllerName(operation) {
            ...
            var routeBuilder = {
                ...
                whenList: function whenList(resolveFns) {
                ...
                whenNew: function whenNew(resolveFns) {
                ...
                whenEdit: function whenEdit(resolveFns) {
                ...
                whenOther: function whenOther(resolveFns, name, suffixUrl) {
                ...
                whenView: function whenView(resolveFns) {
                ...
                otherwise: function otherwise(params) {
    ...
    function portalUIRouteProvider($stateProvider) {
        ...
        **/
        ...
        this.statesFor = function (resourceName) {
            ...
            var stateTemplateUrl = function stateTemplateUrl(statePath) {
                return '/js/Views/' + resourceName.toLowerCase() + '/' + templateFile(statePath) + '.tpl.html';
            };

            var stateUrl = function stateUrl(statePath) {
             ...
             **/
            var stateControllerName = function stateControllerName(statePath) {
             ...
             */
            var templateFile = function templateFile(statePath) {
            ...
            var stateBuilder = {
                ...
                state: function state(statePath, resolveFns) {
...
angular.module('PortalApp').factory('ptBaseResource', function ($resource) {
    ...
    var PtBaseResource = function PtBaseResource(apiName, key, paramDefaults, actions) {
...
angular.module('PortalApp').factory('CheckRequest', function (ptBaseResource, BusinessCheck) {
    ...
    checkRequest.prototype.getTotalAmount = function () {
        if (this.Checks) {
            ...
            var _checks = _.map(this.Checks, function (o) {
                return checkRequest.CType(o, BusinessCheck);
            });
            /************************************************/

            return _.sum(_.filter(_checks, function (o) {
                return !o.isVoid();
            }), 'Amount');
        }
...
angular.module('PortalApp').factory('CorpEntity', function (ptBaseResource, LeadsInfo) {
    ...
    var corpEntity = ptBaseResource('CorporationEntities', 'EntityId', null, { assign: { url: '/api/CorporationEntities/:EntityId/BBLE', method: 'Post' } });
...
.factory('DocNewVersionConfig', function () {
    ...
    var docNewVersionConfig = function docNewVersionConfig() {
...
.factory('DocSearchEavesdropper', function (DocNewVersionConfig) {
    var docSearchEavesdropper = function docSearchEavesdropper() {};
    ...
    docSearchEavesdropper.prototype.endorse = function (revFunc) {
        ...
        if (typeof revFunc != 'function') console.error("set rev function have been set up");
    ...
    docSearchEavesdropper.prototype.start2Eaves = function () {
        ...
        if (this.endorseCheckDate(DocNewVersionConfig.getInstance().date) || this.endorseCheckVersion()) {
            this.revFunc(true);
        } else {
            this.revFunc(false);
        }
...
angular.module('PortalApp').factory('DxGridModel', function ($location, $routeParams) {
    ...
    var dxGridModel = function dxGridModel(opt, texts) {
    ...
    var EditingModel = function EditingModel(texts) {
        this.editMode = "cell";
        if (texts) {
            this.texts = texts;
        }
...
angular.module('PortalApp').factory('LeadResearch', function ($http, LeadsInfo) {
    ...
    var leadResearch = function leadResearch() {};

    leadResearch.prototype.getOwnerSSN = function (BBLE) {
        var self = this;
        $http.post("/api/homeowner/ssn/" + BBLE, JSON.stringify(self.ownerName)).success(function (ssn, status, headers, config) {
            self.ownerSSN = ssn;
        });
    };
...
angular.module('PortalApp').factory('LeadsInfo', function (ptBaseResource) {
    ...
    var leadsInfo = ptBaseResource('LeadsInfo', 'BBLE', null, { verify: { url: '/api/LeadsInfo/Verify' } });
...
angular.module('PortalApp').factory('NewOfferListGrid', function ($http) {
    var _class = function _class(data) {
        ...
        var opt = {
            ...
            columns: [{
                ...
                cellTemplate: function cellTemplate(container, options) {
                    $("<i>").attr("class", "fa fa-history").attr("style", "cursor:pointer").on('dxclick', function () {
    ...
    _class.prototype.cellTemplate = function (container, options) {
        $('<a/>').addClass('dx-link-MyIdealProp').text(options.value).on('dxclick', function () {
...
angular.module('PortalApp').factory('Team', function ($http) {
    ...
    _class.getTeams = function (successCall) {
        ...
        $http.get('/api/CorporationEntities/Teams').success(successCall);
    };
...
angular.module('PortalApp').factory('ptUnderwriter', ['$http', 'ptBaseResource', 'DocSearch', 'LeadsInfo', function ($http, ptBaseResource, DocSearch, LeadsInfo) {
    ...
    var UnderwritingModel = function UnderwritingModel() {
    ...
    var insurancePolicyCalculation = function insurancePolicyCalculation(v1, policyCol, cosCol, fromCol, toCol) {
        if (v1 < fromCol[0]) {
            return 402;
        } else {
            if (v1 <= toCol[0]) return (v1 - fromCol[0]) * policyCol[0];else {
                if (v1 <= toCol[1]) return (v1 - fromCol[1]) * policyCol[1] + cosCol[1];else {
                    if (v1 <= toCol[2]) return (v1 - fromCol[2]) * policyCol[2] + cosCol[1];else {
                        if (v1 <= toCol[3]) return (v1 - fromCol[3]) * policyCol[3] + cosCol[2];else {
                            if (v1 <= toCol[4]) return (v1 - fromCol[4]) * policyCol[4] + cosCol[3];else {
                                if (v1 <= toCol[5]) return (v1 - fromCol[5]) * policyCol[5] + cosCol[4];else {
                                    if (v1 <= toCol[6]) return (v1 - fromCol[6]) * policyCol[6] + cosCol[5];else {
                                        if (v1 <= toCol[7]) return (v1 - fromCol[7]) * policyCol[7] + cosCol[6];
                                    }
                                }
                            }
                        }
     ...
     */
    var RentalHelper = function RentalHelper(isRented, rentalTime, model) {
        var i,
            j,
            k = 0,
            temp = model.NetMontlyRent;
        this._model = [];
        this._model[k] = {
            Month: k + 1,
            Rent: 0,
     ...
     */
    var mapData = function mapData(d) {
     ...
     */
    var applyFixedRules = function applyFixedRules(d) {
        ...
        d.InsurancePremium.CostOwnersPolicy = _.zip(d.InsurancePremium.From, d.InsurancePremium.To, d.InsurancePremium.OwnersPolicyRate).reduce(function (cum, v) {
            var l = cum.length;
        ...
        d.InsurancePremium.CostLoanPolicy = _.zip(d.InsurancePremium.From, d.InsurancePremium.To, d.InsurancePremium.LoanPolicyRate).reduce(function (cum, v) {
            var l = cum.length;
            cum[l - 1] = (v[1] - v[0]) * v[2] + cum[l - 1];
            cum[l] = cum[l - 1];
            return cum;
    ...
    */
    underwriter.applyRule = function (d) {
        ...
        var int = parseInt;
        // PropertyInfo
        d.PropertyInfo.PropertyType = function () {
            return (/.*(A|B|C0|21|R).*/.exec(d.PropertyInfo.TaxClass) ? "Residential" : "Not Residential"
            );
        }();
        ...
        d.Liens.RelocationLien = function () {
            ...
            if (!d.LienCosts.RelocationLienDate) return 0.0;else {
...
angular.module('PortalApp').factory('UnderwritingRequest', ['$http', 'ptBaseResource', function ($http, ptBaseResource) {
...
angular.module("PortalApp").directive('newDsSummary', function () {
    return {
        restrict: 'E',
        scope: {
            summary: '=',
...
angular.module("PortalApp").service('ptContactServices', ['$http', 'limitToFilter', function ($http, limitToFilter) {
    ...
    (function () {
        if (!allContact) {
            $http.get('/Services/ContactService.svc/LoadContacts').success(function (data, status) {
                allContact = data;
        ...
        if (!allTeam) {
            $http.get('/Services/TeamService.svc/GetAllTeam').success(function (data, status) {
                allTeam = data;
            }).error(function (data, status) {
                allTeam = [];
            });
    ...
    this.getAllContacts = function () {
        ...
        return $http.get('/Services/ContactService.svc/LoadContacts').then(function (response) {
    ...
    this.getContacts = function (args, /* optional */groupId) {
        ...
        return $http.get('/Services/ContactService.svc/GetContacts?args=' + args, { noIndicator: true }).then(function (response) {
            if (groupId) return limitToFilter(response.data.filter(function (x) {
    ...
    this.getContactsByID = function (id) {
        if (allContact) return allContact.filter(function (o) {
            return o.ContactId == key;
        });
        return $http.get('/Services/ContactService.svc/GetAllContacts?id=' + id).then(function (response) {
            return limitToFilter(response.data, 10);
        });
    };
    this.getContactsByGroup = function (groupId) {
        if (allContact) return allContact.filter(function (x) {
            return x.GroupId == groupId;
        });
    };
    ...
    this.getContact = function (id, name) {
        if (allContact) return allContact.filter(function (o) {
            if (o.Name && name) return o.ContactId == id && o.Name.trim().toLowerCase() === name.trim().toLowerCase();
        })[0] || {};
        return {};
    };
    ...
    this.getContactByName = function (name) {
        if (allContact) return allContact.filter(function (o) {
            if (o.Name && name) return o.Name.trim().toLowerCase() === name.trim().toLowerCase();
        })[0];
        return {};
    };
    ...
    this.getEntities = function (name, status) {
        ...
        name = name ? '' : name;
        return $http.get('/Services/ContactService.svc/GetCorpEntityByStatus?n=' + name + '&s=' + status).then(function (res) {
    ...
    this.getTeamByName = function (teamName) {
        if (allTeam) {
            return allTeam.filter(function (o) {
                if (o.Name && teamName) return o.Name.trim() == teamName.trim();
            })[0];
        }
        return {};
    };
}]);
angular.module("PortalApp").factory('ptEntityService', ['$http', function ($http) {
    return {
        getEntityByBBLE: function getEntityByBBLE(bble, callback) {
...
angular.module("PortalApp").factory('ptHomeBreakDownService', ["$http", function ($http) {
    return {
        loadByBBLE: function loadByBBLE(bble, callback) {
        ...
        save: function save(bble, data, callback) {
...
angular.module("PortalApp").factory('ptLegalService', function () {
    return {
        load: function load(bble, callback) {
        ...
        savePreQuestions: function savePreQuestions(bble, createBy, data, callback) {
...
angular.module("PortalApp").factory('PortalHttpInterceptor', ['$log', '$q', '$timeout', 'ptCom', function ($log, $q, $timeout, ptCom) {
    ...
    var myInterceptor = {
        ...
        BuildAjaxErrorMessage: function BuildAjaxErrorMessage(response) {
            var message = "";
            /*Only error handle*/
        ...
        BuildErrorMessgeStr: function BuildErrorMessgeStr(messageObj) {

            return 'Error : ' + messageObj.Message || 'No Message' + '<br/> <small>(' + messageObj.urlName || 'No additional Info' + ')</small>';
        }, ErrorUrl: function ErrorUrl(url) {
            return url.toString().replace(/\//g, ' ').replace('api', '');
        },
        BuildErrorMessage: function BuildErrorMessage(data) {
            var urlName;
            if (data) {
                if (data.data) {
                    var messageObj = {
...
angular.module("PortalApp").service('ptShortsSaleService', ['$http', function ($http) {
    ...
    this.getBuyerTitle = function (bble, callback) {
        ...
        $http.get(url + bble).then(function succ(res) {
            if (callback) callback(null, res);
        }, function error() {
...
angular.module("PortalApp").service("ptTime", [function () {
        ...
        this.isPassByMonths = function (start, end, count) {
                ...
                if (months > count) return true;else return false;
        };
...
angular.module("PortalApp").filter('booleanToString', function () {
    ...
    return function (v) {
        if (v == undefined) return "N/A";else if (v) return "Yes";else return "No";
    };
});
...
angular.module("PortalApp").filter('unsafe', ['$sce', function ($sce) {
  return $sce.trustAsHtml;
}]);
angular.module("PortalApp").directive('auditLogs', ['AuditLog', function (AuditLog) {
    return {
        restrict: 'E',
...
angular.module("PortalApp").directive('bindId', ['ptContactServices', function (ptContactServices) {
    return {
        restrict: 'A',
 ...
 * @*********************************************************
 */
angular.module("PortalApp").directive('initGrid', ['$parse', function ($parse) {
    return {
        link: function link(scope, element, attrs, ngModelController) {
            ...
            if (gridOptions) {
                ...
                if (array == null || array == undefined) eval('scope.' + option + '=[];');
...
angular.module("PortalApp").directive('inputMask', function () {
    return {
        restrict: 'A',
        link: function link(scope, el, attrs) {
 ...
 */
angular.module("PortalApp").directive('numberMask', function () {
    return {
        restrict: 'A',
        link: function link(scope, el, attrs) {
 ...
 * @returns {type} 
 */
angular.module("PortalApp").directive('preCondition', function () {
    return {
        require: 'ngModel',
        link: function link(scope, element, attrs, ngModelController) {
            scope.$watch(attrs.preCondition, function (newVal, oldVal) {
                if (!newVal) eval('scope.' + attrs.ngModel + '=null');
            }, true);
        }
    };
});
angular.module("PortalApp").directive('ptAdd', function () {
    return {
        restrict: 'E',
        template: '<i class="fa fa-plus-circle icon_btn text-primary tooltip-examples" title="Add"></i>'
    };
});
angular.module("PortalApp").directive('ptCollapse', function () {
    return {
        restrict: 'E',
        template: '<i class="fa fa-compress icon_btn text-primary" ng-show="!model" ng-click="model=!model"></i>' + '<i class="fa fa-expand icon_btn text-primary" ng-show="model" ng-click="model=!model"></i>',
        scope: {
            model: '='
        },
        link: function postLink(scope, el, attrs) {
...
angular.module("PortalApp").directive('ptDel', function () {
    return {
        restrict: 'E',
        template: '<i class="fa fa-times icon_btn text-danger tooltip-examples" title="Delete"></i>'
    };
});
angular.module("PortalApp").directive('ptEditor', [function () {
    return {
...
angular.module("PortalApp").directive('ptFile', ['ptFileService', '$timeout', function (ptFileService, $timeout) {
    return {
        restrict: 'E',
...
angular.module("PortalApp").directive('ptFiles', ['$timeout', 'ptFileService', 'ptCom', function ($timeout, ptFileService, ptCom) {
    return {
        restrict: 'E',
        ...
        link: function link(scope, el, attrs) {
            ...
            $(el).find('.drop-area').on('dragenter', function (e) {
                e.preventDefault();
...
angular.module("PortalApp").directive('ptFinishedMark', [function () {
    return {
        restrict: 'E',
...
angular.module("PortalApp").directive('ptInitBind', function () {
...
angular.module("PortalApp").directive('ptInitModel', function () {
    return {
        restrict: 'A',
        require: '?ngModel',
        priority: 99,
...
angular.module("PortalApp").directive('ptLink', ['ptFileService', function (ptFileService) {
    return {
        restrict: 'E',
        scope: {
            ptModel: '='
...
angular.module("PortalApp").directive('ptRadio', function () {
    return {
        restrict: 'E',
        template: '<input type="checkbox" id="{{name}}Y" ng-model="model" class="ss_form_input" ng-disabled="ngDisabled">' + '<label for="{{name}}Y" class="input_with_check"><span class="box_text">{{trueValue}}&nbsp</span></label>' + '<input type="checkbox" id="{{name}}N" ng-model="model" ng-true-value="false" ng-false-value="true" class="ss_form_input" ng-disabled="ngDisabled">' + '<label for="{{name}}N" class="input_with_check"><span class="box_text">{{falseValue}}&nbsp</span></label>',
        scope: {
            model: '=',
            name: '@',
            defaultValue: '@',
...
angular.module("PortalApp").directive('radioInit', function () {
    return {
        restrict: 'A',
        link: function link(scope, el, attrs) {
...
angular.module("PortalApp").directive('ssDate', function () {
    return {
        restrict: 'A',
        scope: true,
...
angular.module("PortalApp").controller('BuyerEntityCtrl', ['$scope', '$http', 'ptContactServices', 'CorpEntity', function ($scope, $http, ptContactServices, CorpEntity) {
    $scope.EmailTo = [];
    $scope.EmailCC = [];
    ...
    $http.get('/Services/TeamService.svc/GetAllTeam').success(function (data) {
        $scope.AllTeam = data;
    }).error(function (data) {
    ...
    $scope.Groups = [{ GroupName: 'All Entities' }, { GroupName: 'Available' }, { GroupName: 'Assigned Out' }, {
        ...
        SubGroups: [{ GroupName: 'NHA Current Offer' }, { GroupName: 'Isabel Current Offer' }, { GroupName: 'Quiet Title Action' }, { GroupName: 'Deed Purchase' }, { GroupName: 'Straight Sale' }, { GroupName: 'Jay Current Offer' }]
    }, {
        GroupName: 'Sold',
        SubGroups: [{ GroupName: 'Purchased' }, { GroupName: 'Partnered' }, { GroupName: 'Sold (Final Sale)/Recyclable' }]
    }, { GroupName: 'In House' }, { GroupName: 'Agent Corps' }, { GroupName: 'Not for Use' }, { GroupName: 'Reserve' }];
    ...
    $scope.GroupCount = function (g) {
        ...
        if (g.GroupName == 'All Entities') {
            if ($scope.SelectedTeam) {
                return $scope.CorpEntites.filter(function (o) {
                    return o.Office && o.Office.toLowerCase().trim() == $scope.SelectedTeam.toLowerCase().trim();
                }).length;
            }
        ...
        var corps = $scope.CorpEntites.filter(function (o) {
            return o.Status && o.Status.toLowerCase().trim() == g.GroupName.toLowerCase().trim();
        });
        if ($scope.SelectedTeam) {
            corps = corps.filter(function (o) {
                return o.Office && o.Office.toLowerCase().trim() == $scope.SelectedTeam.toLowerCase().trim();
            });
        }
        return corps.length;
    ...
    $scope.TeamCount = function (teamName) {
        ...
        var crops = [];
        crops = teamName ? $scope.CorpEntites.filter(function (o) {
            return o.Office && o.Office.toLowerCase().trim() == teamName.toLowerCase().trim();
        }) : $scope.CorpEntites;

        if ($scope.selectType && $scope.selectType != $scope.Groups[0].GroupName) {
            crops = crops.filter(function (o) {
                return o.Status && o.Status.toLowerCase().trim() == $scope.selectType.toLowerCase().trim();
            });
        }
            ...
            load: function load(loadOptions) {
                if (loadOptions.searchValue) {
                    ...
                    return employees.filter(function (e) {
                        e.Email && (e.Email.toLowerCase() == q.toLowerCase() || e.Name.toLowerCase() == q.toLowerCase());
                    }).slice(0, 10);
                }
                return employees.slice(0, 10);
            },
            byKey: function byKey(key, extra) {
                // . . .
            }
    ...
    $scope.EntitiesFilter = function (entity) {
        if ($scope.selectType == 'All Entities' || entity.Status && $scope.selectType.toLowerCase().trim() == entity.Status.toLowerCase().trim()) return true;
        var subs = $scope.Groups.filter(function (o) {
            return o.GroupName == $scope.selectType;
        })[0];
    ...
    $scope.AllGroups = function () {
        var HasSub = $scope.Groups.filter(function (o) {
            return o.SubGroups != null;
        });
        var groups = [];
        for (var i = 0; i < HasSub.length; i++) {
            groups = groups.concat(HasSub[i].SubGroups);
        }
        var HasNotSub = $scope.Groups.filter(function (o) {
            return o.SubGroups == null && o.GroupName != 'All Entities';
        });
...
angular.module('PortalApp').controller('ConstructionCtrl', ['$scope', '$http', '$interpolate', 'ptCom', 'ptContactServices', 'ptEntityService', 'ptShortsSaleService', 'ptLeadsService', 'ptConstructionService', function ($scope, $http, $interpolate, ptCom, ptContactServices, ptEntityService, ptShortsSaleService, ptLeadsService, ptConstructionService) {

    //data structure defination
    var CSCaseModel = function CSCaseModel() {
        this.CSCase = {
            InitialIntake: {},
            Photos: {},
            Utilities: {
    ...
    var PercentageModel = function PercentageModel() {
        this.intake = {
            count: 0,
            finished: 0
        };
    ...
    $scope.ChangeStatus = function (scuessfunc, status) {
        $http.post('/api/ConstructionCases/ChangeStatus/' + leadsInfoBBLE, status).success(function () {
            if (scuessfunc) {
                scuessfunc();
            } else {
    ...
    $scope.fetchDOBViolations = function () {
        ptCom.confirm("<h3 style='color:red'>Warning!!!</h3>Getting information from DOB takes a while.<br>And it will <b>REPLACE</b> your current Data, are you sure to continue?", "Warning").then(function (confirmed) {
            if (confirmed) {
                ptConstructionService.getDOBViolations($scope.CSCase.BBLE, function (error, res) {
                    if (error) {
                        ...
                        ptCom.confirm("<h3 style='color:red'>Warning!!!</h3>Your current DOB Violation data will be replaced.", "Warning").then(function (confirmed) {
                            if (confirmed) {
 ...
 */
angular.module('PortalApp').controller('LeadTaxSearchCtrl', function ($scope, $http, $element, $timeout, ptContactServices, ptCom, DocSearch, LeadsInfo, DocSearchEavesdropper, DivError) {
    ...
    $scope.newVersionValidate = function () {
        ...
        var validateFields = ["Has_Deed_Purchase_Deed", "Has_c_1st_Mortgage_c_1st_Mortgage", "fha", "Has_c_2nd_Mortgage_c_2nd_Mortgage", "has_Last_Assignment_Last_Assignment", "fannie", "Freddie_Mac_", "Has_Due_Property_Taxes_Due", "Has_Due_Water_Charges_Due", "Has_Open_ECB_Violoations", "Has_Open_DOB_Violoations", "hasCO", "Has_Violations_HPD_Violations", "Is_Open_HPD_Charges_Not_Paid_Transferred", "has_Judgments_Personal_Judgments", "has_Judgments_HPD_Judgments", "has_IRS_Tax_Lien_IRS_Tax_Lien", "hasNysTaxLien", "has_Sidewalk_Liens_Sidewalk_Liens", "has_Vacate_Order_Vacate_Order", "has_ECB_Tickets_ECB_Tickets", "has_ECB_on_Name_ECB_on_Name_other_known_address",
        ...
        "Has_Other_Mortgage", "Has_Other_Liens", "Has_TaxLiensCertifcate", "Has_COS_Recorded", "Has_Deed_Recorded"];
        var checkedAttrs = [["Has_Other_Mortgage", "OtherMortgage"], ["Has_Other_Liens", "OtherLiens"], ["Has_TaxLiensCertifcate", "TaxLienCertificate"], ["Has_COS_Recorded", "COSRecorded"], ["Has_Deed_Recorded", "DeedRecorded"]];
...
angular.module('PortalApp').controller('LegalCtrl', ['$scope', '$http', 'ptContactServices', 'ptCom', 'ptTime', '$window', function ($scope, $http, ptContactServices, ptCom, ptTime, $window) {
    ...
    $scope.GetContactById = function (id) {
        return AllContact.filter(function (o) {
            return o.ContactId == id;
        })[0];
    };
    ...
    $scope.CompleteResearch = function () {
        ...
        $http.post('LegalUI.aspx/CompleteResearch', data).success(function () {
            ...
            if (typeof gridTrackingClient !== 'undefined') gridTrackingClient.Refresh();
        }).error(function (data) {
    ...
    $scope.LoadLeadsCase = function (BBLE) {
        ...
        $http.post('LegalUI.aspx/GetCaseData', data).success(function (data, status, headers, config) {
            ...
            $scope.LogChange = {
                'TaxLienFCStatus': { "old": $scope.LegalCase.TaxLienFCStatus, "now": function now() {
                        return $scope.LegalCase.TaxLienFCStatus;
                    }, "msg": 'Tax Lien FC status changed from ' },
                'CaseStauts': { "old": $scope.LegalCase.CaseStauts, "now": function now() {
                        return $scope.LegalCase.CaseStauts;
        ...
        $http.get(shortsaleUrl).success(function (data) {
        ...
        $http.get(taxlienUrl).success(function (data) {
            $scope.TaxLiens = data;
        ...
        $http.get(legalecoursUrl).success(function (data) {
    ...
    $scope.ShowContorl = function (m) {
        var t = typeof m === 'undefined' ? 'undefined' : _typeof(m);
        if (t === "string") {
            return m === 'true';
        }
        return m;
    ...
    $scope.DocGenerator = function (tplName) {
        ...
        var Tpls = [{
            ...
            data: {
                ...
                "Defendants": $scope.LegalCase.SecondaryInfo.OSC_Defendants ? ',' + $scope.LegalCase.SecondaryInfo.OSC_Defendants.map(function (o) {
                    return o.Name;
                }).join(",") : ' ',
            ...
            data: {
                ...
                "Defendants": $scope.LegalCase.SecondaryInfo.DeedReversionDefendants ? ',' + $scope.LegalCase.SecondaryInfo.DeedReversionDefendants.map(function (o) {
                    return o.Name;
                }).join(",") : ' ',
                "CourtAddress": $scope.GetCourtAddress($scope.LeadsInfo.Borough),
                "PropertyAddress": $scope.LeadsInfo.PropertyAddress
            ...
            data: {
                ...
                "Defendants": $scope.LegalCase.SecondaryInfo.SPComplaint_Defendants ? ',' + $scope.LegalCase.SecondaryInfo.SPComplaint_Defendants.map(function (o) {
                    return o.Name;
                }).join(",") : ' ',
                "CourtAddress": $scope.GetCourtAddress($scope.LeadsInfo.Borough),
                "PropertyAddress": $scope.LeadsInfo.PropertyAddress
            ...
            data: {
                ...
                "Defendants": $scope.LegalCase.SecondaryInfo.QTA_Defendants ? ',' + $scope.LegalCase.SecondaryInfo.QTA_Defendants.map(function (o) {
                    return o.Name;
                }).join(",") : ' ',
                "CourtAddress": $scope.GetCourtAddress($scope.LeadsInfo.Borough),
                "PropertyAddress": $scope.LeadsInfo.PropertyAddress,
        ...
        var tpl = Tpls.filter(function (o) {
            return o.tplName == tplName;
        })[0];
    ...
    $scope.CheckSecondaryTags = function (tag) {
        return $scope.LegalCase.SecondaryTypes.filter(function (t) {
            return t == tag;
        })[0];
    };
    ...
    $scope.missingItems = [{ id: 1, label: "Mortgage" }, { id: 2, label: "Note" }, { id: 3, label: "Assignment" }];
    ...
    $scope.checkMissInCertValue = function () {
        ...
        if (!$scope.LegalCase.ForeclosureInfo.MissInCert || $scope.LegalCase.ForeclosureInfo.MissInCert.length == 0) return true;else return false;
    };
...
var CONSTANT_ASSIGN_PARTIES_GRID_OPTION = {
    ...
    initRowText: function initRowText() {
        self = this;
...
var CONSTANT_ASSIGN_LIST_GRID_OPTION = {
    ...
    onRowPrepared: function onRowPrepared(rowInfo) {
        if (rowInfo.rowType != 'data') return;
        rowInfo.rowElement.addClass('myRow');
    },
    columnAutoWidth: true,
...
portalApp.controller('preAssignFinanceCtrl', function ($scope, PreSignFinaceList) {
    ...
    $scope.preSignRecordsGridOpt.masterDetail = {
        ...
        template: function template(container, options) {
            ...
            $("<div>").addClass("internal-grid").dxDataGrid(opt).appendTo(container);
        }
    };
...
angular.module('PortalApp').controller("ReportWizardCtrl", function ($scope, $http, $timeout, ptCom) {
    ...
    $scope.reload = function (callback) {
        $scope.step = 1;
        $scope.CurrentQuery = null;
        $http.get(CUSTOM_REPORT_TEMPLATE).then(function (res) {
    ...
    $scope.loadSavedReport = function () {
        $http.get("/api/Report/Load").then(function (res) {
    ...
    $scope.load = function (q) {
        $scope.reload(function () {
            if (q.ReportId) {
                $http.get("/api/Report/Load/" + q.ReportId).then(function (res) {
                    var data = res.data;
...
angular.module("PortalApp").controller('ShortSaleCtrl', ['$scope', '$http', '$timeout', 'ptContactServices', 'ptCom', function ($scope, $http, $timeout, ptContactServices, ptCom) {
    $scope.ptContactServices = ptContactServices;
    ...
    $scope.MoveToConstruction = function (scuessfunc) {
        ...
        $http.post('ShortSaleServices.svc/MoveToConstruction', JSON.stringify(data)).success(function () {
            if (scuessfunc) {
                scuessfunc();
            } else {
    ...
    $scope.MoveToTitle = function (scuessfunc) {
        ...
        $http.post('ShortSaleServices.svc/MoveToTitle', JSON.stringify(data)).success(function () {
            if (scuessfunc) {
                scuessfunc();
            } else {
    ...
    $scope.GetShortSaleCase = function (caseId, callback) {
        ...
        ptCom.startLoading();
        var done1, done2;
        $http.get("ShortSaleServices.svc/GetCase?caseId=" + caseId).success(function (data) {
            $scope.SsCase = data;
    ...
    $scope.NGremoveArrayItem = function (item, index, disable) {
        ...
        if (r) {
            if (disable) item[index].DataStatus = 3;else item.splice(index, 1);
        }
    };
    ...
    $scope.AutoSaveShortSale = function (callback) {
        ...
        $http.post('ShortSaleServices.svc/SaveCase', JSON.stringify(data)).success(function (data) {
            autoSaveError = false;
            ...
            if (!autoSaveError) {
                autoSaveError = true;
                var message = data1 && (typeof data1 === 'undefined' ? 'undefined' : _typeof(data1)) == 'object' && data1.message ? data1.message : JSON.stringify(data1);
                ptCom.alert("Error in AutoSave. status " + status + "Error : " + message);
            }
        });
    };

    var RemoveDeletedMortgages = function RemoveDeletedMortgages() {
        _.remove($scope.SsCase.Mortgages, { DataStatus: 3 });
        console.log($scope.SsCase.Mortgages);
    };

    var SyncObjects = function SyncObjects(obj, toObj) {
        var copy = toObj;

        // Handle Date
        if (obj instanceof Date) {
            if (copy == null) copy = new Date();
        ...
        if (obj instanceof Object) {
            ...
            for (var attr in obj) {
                if (obj.hasOwnProperty(attr)) {
                    if (null == obj[attr] || "object" != _typeof(obj[attr])) {
    ...
    $scope.SaveShortSale = function (callback) {
        ...
        $http.post('ShortSaleServices.svc/SaveCase', JSON.stringify(data)).success(function () {
    ...
    $scope.checkPendingModified = function () {
        ...
        _.each($scope.SsCase.ValueInfoes, function (el, index) {
            if (el.Pending) {
                var oldEl = $scope.oldPendingValues.filter(function (e, i) {
                    return e.$$hashKey == el.$$hashKey;
                })[0];
                if (!oldEl) {
                    for (var property in el) {
...
portalApp.controller('shortSalePreSignCtrl', function ($scope, ptCom, $http, ptContactServices, $location, PortalHttpInterceptor,

/**** Models *****/
PropertyOffer, WizardStep, Wizard, DivError, LeadsInfo, DocSearch, Team, NewOfferListGrid, ScopeHelper, QueryUrl, AssignCorp) {
...
angular.module("PortalApp").controller("TitleController", ['$scope', '$http', 'ptCom', 'ptContactServices', 'ptLeadsService', 'ptShortsSaleService', function ($scope, $http, ptCom, ptContactServices, ptLeadsService, ptShortsSaleService) {
    /* model define*/
    ...
    $scope.completeCase = function () {
        if ($scope.CaseStatus != 1 && $scope.BBLE) {
            ptCom.confirm("You are going to complated the case?", "").then(function (r) {
                if (r) {
                    $http({
                        method: 'POST',
            ...
            ptCom.confirm("You are going to uncomplated the case?", "").then(function (r) {
                if (r) {
                    $http({
                        method: 'POST',
    ...
    $scope.updateCaseStatus = function () {
        if ($scope.CaseStatus && $scope.BBLE) {
            ...
            ptCom.confirm("You are going to change case status?", "").then(function (r) {
                if (r) {
                    $http({
                        method: 'POST',
    ...
    $scope.getStatus = function (bble) {
        $http.get('/api/Title/GetCaseStatus?bble=' + bble).then(function succ(res) {
    ...
    var FeeClearanceModel = function FeeClearanceModel() {
        this.data = [{
            name: 'Purchase Price',
            cost: 0.0,
...
angular.module("PortalApp").controller("UnderwriterController", ['$scope', 'ptCom', 'ptUnderwriter', function ($scope, ptCom, ptUnderwriter) {
    ...
    $scope.feedData = function () {
        $scope.data.PropertyInfo.TaxClass = 'A0', $scope.data.PropertyInfo.ActualNumOfUnits = 1;
...
angular.module("PortalApp").controller('VendorCtrl', ["$scope", "$http", "$element", function ($scope, $http, $element) {
    ...
    $scope.AddGroups = function () {
        $http.post('/CallBackServices.asmx/AddGroups', { gid: $scope.AddGroupId, groupName: $scope.addGroupName, addUser: $('#CurrentUser').val() }).success(function (data) {
            $scope.initGroups();
        });
    };
    $scope.ChangeGroups = function (g) {
        ...
        $http.post('/CallBackServices.asmx/GetContactByGroup', { gId: g.Id }).success(function (data) {
    ...
    $scope.initGroups = function () {
        $http.post('/CallBackServices.asmx/GetAllGroups', {}).success(function (data, status, headers, config) {
            $scope.Groups = data.d;
        }).error(function (data, status, headers, config) {
    ...
    $http.post('/CallBackServices.asmx/GetContact', { p: '1' }).success(function (data, status, headers, config) {
        $scope.InitDataFunc(data);
    ...
    $scope.addContactFunc = function () {
        ...
        debugger;
        $http.post("/CallBackServices.asmx/AddContact", { contact: $scope.addContact }).success(function (data, status, headers, config) {
    ...
    $scope.SaveCurrent = function () {
        ...
        $http.post("/CallBackServices.asmx/SaveContact", { json: $scope.currentContact }).success(function (data, status, headers, config) {
            alert("Save succeed!");
            $scope.initLenderList();
        }).error(function (data, status, headers, config) {
