...
if (!DevExpress.MOD_WIDGETS_WEB) {
    ...
    (function($, DX, undefined) {
        ...
        var DX_MENU_CLASS = 'dx-menu',
            ...
            DX_MENU_ICON_CLASS = 'dx-menu-icon',
        ...
        var dxMenuBase = ui.CollectionContainerWidget.inherit({
                ...
                _renderImage: function(item, $itemContent) {
                    ...
                    if (item.icon)
                        ...
                        $itemImage = $('<span>').attr('src', item.iconSrc).appendTo($itemContent);
                    if ($itemImage)
                        $itemImage.addClass(DX_MENU_ICON_CLASS)
                },
                    ...
                    refresh: function(storeLoadOptions) {
                        ...
                        return foreachCollapsedGroups(that, function(groupInfo) {
                                var groupCountQuery = store.totalCount({filter: createGroupFilter(groupInfo.path, storeLoadOptions)}),
                                    groupOffsetQuery = store.totalCount({filter: createOffsetFilter(groupInfo.path, storeLoadOptions)});
    ...
    (function($, DX, undefined) {
        ...
        var ColumnsController = dataGrid.Controller.inherit(function() {
                ...
                return {
                        ...
                        columnOption: function(identificator, optionName, value) {
                            var that = this,
                                i,
                                changeType,
                                columns = that._columns,
                            ...
                            if (column) {
                                ...
                                if (utils.isString(optionName)) {
                                    ...
                                    if (optionGetter(column, {functionsAsIs: true}) !== value) {
                                        if (optionName === 'groupIndex') {
                                            moveColumnToGroup(that, column, value);
                                            changeType = 'grouping'
                                        }
                                    ...
                                    if (optionName.groupIndex !== undefined) {
                                        moveColumnToGroup(that, column, optionName.groupIndex);
                                        changeType = 'grouping'
                                    }
                                    else if (optionName.sortIndex !== undefined || optionName.sortOrder !== undefined)
                                        changeType = 'sorting';
                                    else
                                        changeType = 'columns';
                                ...
                                if (changeType)
                                    that.columnsChanged.fire({
                                        changeType: changeType,
                                        columnIndex: column.initialIndex,
                                        ...
                                        update: function() {
                                            ...
                                            if (dataSource)
                                                if (utils.isObject(dataSource) || utils.isArray(dataSource)) {
                                                    dataSource = new DX.data.DataSource($.extend(true, {paginate: false}, dataUtils.normalizeDataSourceOptions(dataSource)));
    ...
    (function($, DX) {
        ...
        dataGrid.SelectionController = dataGrid.Controller.inherit(function() {
            ...
            var equalKeys = function(key1, key2, level) {
                    var propertyName,
                        i;
                    level = level || 0;
                    if (level < MAX_EQUAL_KEYS_LEVEL)
                        if (utils.isObject(key1) && utils.isObject(key2)) {
                            for (propertyName in key1)
                                if (key1.hasOwnProperty(propertyName) && !equalKeys(key1[propertyName], key2[propertyName], level + 1))
                                    return false;
                            ...
                            for (i = 0; i < key1.length; i++)
                                if (!equalKeys(key1[i], key2[i], level + 1))
                                    return false;
                            return true
                        }
                            ...
                            if (column.command === 'select') {
                                ...
                                groupElement = $('<div />').addClass(DATAGRID_CHECKBOX_SIZE_CLASS).dxCheckBox({
                                    ...
                                    valueChangeAction: function(e) {
                                        if (e.jQueryEvent && getSelectAllValue(selectionController) !== e.value) {
    ...
    (function($, DX) {
        ...
        var FILTERING_TIMEOUT = 700,
            ...
            DATAGRID_UPDATE_FOCUS_EVENTS = addNamespace('focusout dxpointerdown dxclick', DATAGRID_MODULE_NAMESPACE);
        ...
        dataGrid.EditorFactoryController = dataGrid.Controller.inherit(function() {
                        ...
                        valueChangeAction: function(e) {
                            var updateValue = function(e, isTimeout) {
                                    if (!options.validate || options.validate(e.value))
                                        options.value(options.parseValue ? options.parseValue(e.value) || "" : e.value);
                                    else if (!isTimeout)
            ...
            var createSelectBox = function($container, options) {
                    ...
                    if (lookup) {
                        ...
                        $container.dxSelectBox({
                            ...
                            showClearButton: lookup.allowClearing && !options.filtering,
                            displayExpr: function(data) {
                                if (data === null)
            ...
            return {
                    ...
                    createEditor: function($container, options) {
                        var userCreateEditor = this.option('createEditor');
        ...
        var ColumnHeadersViewFilterRowExtender = {
                ...
                _renderFilterRowCell: function(rootElement, column) {
                    ...
                    var updateFilterText = function(that, column, value) {
                            var filterOptions,
                                filterText = column.dataType === "date" ? DX.formatHelper.format(value, column.format) : value,
                                columnsController = that._columnsController;
                            if (!column.filterText && !filterText)
                                return;
                            column.filterText = filterText;
                            columnsController.columnOption(column.initialIndex, 'filterText', filterText)
                        };
                            ...
                            calculateFilterExpression: function(text, selectedFilterOperation) {
                                ...
                                if (parsedValue !== undefined)
                                    if (column.dataType === 'string')
                                        filter = [dataField, selectedFilterOperation || 'contains', parsedValue];
                                    else if (column.dataType === 'date') {
                                        var dateStart = new Date(parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate()),
                                            dateEnd = new Date(parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate() + 1);
                                        ...
                                        filter = [dataField, selectedFilterOperation || '=', parsedValue];
                                return filter
                            },
                            allowFiltering: true
                        });
        ...
        dataGrid.RowsView = dataGrid.ColumnsView.inherit({
            ...
            _renderContent: function(contentElement, tableElement) {
                ...
                if (virtualItemsCount) {
                    ...
                    DX.translator.move(tableElement, {top: Math.floor(virtualItemsCount.begin * rowHeight)});
                ...
                if (that._freeSpaceRowElement && contentElement) {
                    ...
                    if (that._dataController.items().length > 0)
                        if (resultHeight > 0) {
                            ...
                            if (freespaceRowCount > 0 && that._dataController.pageCount() > 1) {
                    ...
                    if (e.optionName === 'width') {
                        ...
                        if (visibleColumnIndex !== -1) {
                            ...
                            $.each($tables, function(key, table) {
                                $(table).find('col').eq(visibleColumnIndex).css('width', e.column.width)
                            });
    ...
    (function($, DX) {
        ...
        dataGrid.ColumnsResizerViewController = dataGrid.ViewController.inherit({
            ...
            _subscribeToEvents: function() {
                ...
                this._$parentContainer.on(addNamespace('dxpointerdown', DATAGRID_MODULE_NAMESPACE), this, this.createAction(this._startResizing));
